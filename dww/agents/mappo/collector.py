## ================NOTE====================
## Collector implementation from Tianshou with some modification
...
class Collector(object):
    ...
    def reset_old_data(self, a_i):
        self.old_data[a_i] = Batch(
            obs={},
            act={},
            rew={},
            terminated={},
        ...
        while True:
            ...
            for a_i in self.agents_ids:
                ...
                if random:
                    ...
                    if result.get("act", None) is None:
                        print(self.data)
                        print(result)
                ...
                if not self.old_data[a_i].obs.is_empty():
                    self.old_data[a_i].obs_next = self.data.obs
                    next_obs_len = len(self.data.obs)

                    if len(self.old_data[a_i].obs) == next_obs_len:
                        ptr, ep_rew, ep_len, ep_idx = self.buffer.add(
                            self.old_data[a_i], buffer_ids=ready_env_ids
                        )
                        self.reset_old_data(a_i)

                if np.any(done):
                    for a_idx in self.agents_ids:
                        if not self.old_data[a_idx].obs.is_empty():
                            ptr, ep_rew, ep_len, ep_idx = self.buffer.add(
                                self.old_data[a_idx], buffer_ids=ready_env_ids
                            )
                            self.reset_old_data(a_i)
                ...
                self.old_data[a_i] = deepcopy(self.data)
